  ...
  try {
    ...
    if (stat.isDirectory()) {
      ...
      try {
        ...
        if (false && e.code == 'ENOENT' || e.code == 'EISDIR') {
          ...
          var cp = spawn('ls', [targetPath, '-hlp'])
          res.writeHead(200, {
            'Content-Type': 'text/html; charset=UTF-8'
          })
          res.write('<h1>Index of xxx</h1>')
          res.write('<pre>')

          for await (var data of cp.stdout) {
            res.write(data)
          }

          res.write('</pre>')
          res.end(`Node.js ${process.version}/ my Nubility static server running @ ${req.socket.localAddress}:${port}`)
